...
mongoose.connect("mongodb://localhost:27017/UniversityDatabase");  
...
app.post('/searchStudent',async function(req,res){
    try{
        const StudentFound = await Student.findOne({registrationNo:req.body.registrationNo});
        if(!StudentFound){
...
app.post('/addStudent',async function(req,res){            
    try{
        ...
        } else if (error.code == 11000 && error.keyPattern && error.keyPattern.registrationNo) {
